...
// be a rune or a Key.
func (g *Gui) SetKeybinding(viewname string, key interface{}, mod Modifier, handler func(*Gui, *View, interface{}) error) error {
	var kb *keybinding
...
func (g *Gui) SetEventHandler(viewname string, handler func(*Gui, *View, interface{}) error) error {
	var kb *keybinding

	kb = newKeybinding(viewname, 0, 0, 0, handler)
	g.eventhandlers = append(g.eventhandlers, kb)
	return nil
}
...
func (g *Gui) execKeybindings(v *View, ev *termbox.Event) (matched bool, err error) {
	matched = false
	for _, kb := range g.eventhandlers {
		if kb.handler == nil {
			continue
		}
		if kb.matchView(v) {
			if err := kb.handler(g, v, ev); err != nil {
				return false, err
			}
		}
	}
